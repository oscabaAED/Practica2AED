...
grafico3 <- grafico2 + ggtitle("Relación entre temperatura máxima y mínima y probabilidad de lluvia")
...
grafico3 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, color = RainToday)) + geom_point()
# En la representación por color se distinguen mejor los valores que en la representación por formas

grafico4 <- ggplot(ds, aes(x = Temp9am, y = Pressure9am, color = RainToday)) + geom_point() + labs(
  title = "Probabilidad de lluvia según condiciones meteorológicas",
  subtitle = "Lluvia en función de temperatura y presión a las 9 AM",
  x = "Temperatura 9 AM (ºC)",
  y = "Presión atmosférica 9 AM (mb)",
  colour='Lluvia')
...
iris.grafico3 <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+geom_point() + facet_grid(Measure~.)
iris.grafico4 <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+geom_point() + facet_grid(.~Measure)
# La disposición por columnas es mejor para comparar valores

iris.grafico5 <- ggplot(iris.tidy, aes(x = Species, y = Value, color = Part))+geom_jitter() + facet_grid(.~Measure)
# Con geom_jitter los puntos tienen cierto ruido para observar mejor aquellos que caen en el mismo sitio, dando mejor idea de las cantidades de puntos.
...
# Aunque se pueden distinguir los fabricantes, falta añadir un ajuste que nos permita ver la evolución en cada caso

ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante, linetype = Fabricante))+geom_point()+geom_line()

ggplot(DatosEjer4, aes(x = Fecha, y = Ventas, color = Fabricante, linetype = Fabricante))+geom_point()+geom_line()+ggtitle("Ventas mensuales de diferentes compañías tecnológicas")
...
ggplot(Experimento, aes(x=Dosis, y=Altura, shape=Complemento, linetype=Complemento)) + geom_point() + geom_line()

ggplot(Experimento, aes(x=Dosis, y=Altura, shape=Complemento, linetype=Complemento)) + geom_point() + geom_line() + 
  geom_errorbar(aes(ymin=Altura-Error_Altura, ymax=Altura+Error_Altura), width=.1, position=position_dodge(0.05))

ggplot(Experimento, aes(x = Dosis, y = Altura, color = Complemento)) +
  geom_point(aes(shape = Complemento)) +
  geom_line(aes(linetype = Complemento)) +
  geom_errorbar(aes(ymin = Altura - Error_Altura,
...
ggplot(ice.river.df)+geom_line(aes(x = time, y = flow.vat, color = "Vatnsdalsa"))+geom_line(aes(x = time, y = flow.jok, color = "Jokulsa Eystri"))+scale_color_manual(values = c("Vatnsdalsa" = "red", "Jokulsa Eystri" = "blue"))+labs(
  title = "Caudal del río Vatnsdalsa y Jokulsa Eystri de 1972 a 1975",
  x = "Tiempo",
  y = "Caudal (cms)",
  colour="Río")
...
ggplot(ds, aes(x = Year, y = MaxTemp, fill = Year, color = Year)) + geom_violin() + labs(
...
ggplot(df, aes(x = HEIGHT)) + geom_histogram() + labs(
    title = "Histograma de la variable Altura",
    x = "Altura (cm)",
    y = "Frecuencia")

ggplot(df, aes(x = HEIGHT)) + geom_histogram(breaks = seq(155, 195, length.out = 21)) + labs(
    title = "Histograma de la variable Altura",
    x = "Altura (cm)",
    y = "Frecuencia")

ggplot(df, aes(x = HEIGHT)) + geom_histogram(fill = "blue", color = "red", alpha = 0.5, breaks = seq(155, 195, length.out = 21)) + labs(
    title = "Histograma de la variable Altura",
    x = "Altura (cm)",
    y = "Frecuencia")

ggplot(df, aes(x = HEIGHT)) + geom_histogram(aes(y = ..density..), fill = "blue", color = "red", alpha = 0.5, breaks = seq(155, 195, length.out = 21)) + geom_density(col="green") + labs(
    title = "Histograma de la variable Altura",
    x = "Altura (cm)",
    y = "Frecuencia")
